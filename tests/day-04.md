...
|---|---|---|---|
| Required authentication methods are enabled | Microsoft Authenticator, SMS, Temporary Access Pass and Passkey (FIDO2) are available for the intended users or groups | Partial: enabled methods and group counts shown; TAP/Passkey target group names are not expanded | [01-authentication-methods-policy.png](../evidence/day-04/01-authentication-methods-policy.png) |
...
| Temporary Access Pass can be used to bootstrap strong authentication | A time-limited TAP is generated and used for strong-method registration | Partial: one-hour TAP issuance shown; TAP sign-in/consumption is not captured | [03-temporary-access-pass-bootstrap.png](../evidence/day-04/03-temporary-access-pass-bootstrap.png) |
...
| `peter.finance` cannot access Expense Portal with authentication methods that do not satisfy CA003 | Access is denied or additional stronger authentication is required | Partial: Peter's stronger-methods-required block shown; app and CA003 attribution not visible | [06-peter-weak-authentication-denied.png](../evidence/day-04/06-peter-weak-authentication-denied.png) |
...
The recorded evidence supports Anna's passkey authentication, application access and CA001/CA003 evaluation. TAP bootstrap use and attribution of Peter's block remain partially verified; configuration alone does not resolve them.
